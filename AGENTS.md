...
- 包元数据当前位于 `pyproject.toml`：包名 `apex-flow`，开发版本 `2.0.0.dev0`，目标发布版本 `2.0.0`，Python `>=3.10`，命令行入口 `apex = apex.__main__:main`，许可证 LGPLv3。项目不使用 uv 项目管理或 `uv.lock`；开发和 CI 可使用 `uv pip` 安装环境。
- 当前 GitHub Actions CI 使用 Python `3.11` 和 pytest-cov 覆盖率报告；规范测试命令见 `docs/development/testing.md` 和 `.github/workflows/main.yml`。
...
- 非平凡改动前，人类和 Codex 应先确认目标、架构、公开接口、迁移影响和测试证据，再修改代码；具体 workplan 要求见 `docs/development/collaboration-sop.md`。
...
- APEX 2.0 工程方向见 `docs/development/apex-2-roadmap.md`；本文只保留协作时必须遵守的边界。
- 新的性质 workflow、结构建模、Skill 化使用、workflow 后端解耦、依赖现代化和 CI/CD 改动都必须保留现有 CLI/JSON 工作流兼容边界，经过人类 review，并用确定性测试或明确的真实验证证据支撑。
- 不要猜测材料结构，不要引入隐藏 fallback，也不要在没有迁移路径的情况下改变公开参数键、输出目录约定或 dflow/Argo 支持边界。

## 测试与证据

- 规范测试运行器、pytest-cov 命令、fixture 要求、mock 规则和真实 workflow 验证边界见 `docs/development/testing.md`。
- 存量 `unittest.TestCase` 测试可以保留；新增测试以及 AI Agent 后续编写的测试应使用 pytest 风格。
- 修改 workflow graph、OP、新性质、calculator 或 property 逻辑时，必须用测试证据证明本地、dflow、dispatcher 和 archive 相关路径没有被意外改变。
...
- 提交应保持小范围，聚焦一个 bug fix、功能切片或文档更新；具体编辑前、实现中和收尾要求见 `docs/development/collaboration-sop.md`。
